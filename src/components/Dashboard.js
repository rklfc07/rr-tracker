...
const EMPTY_FORM = { name: "", category: "Google Ads", client_name: "", color: "#c97d2a", budget: "", received: "" };
...
export default function Dashboard({ projects, onSelectProject, onRefresh }) {
  ...
  const openEdit = (e, p) => {
    e.stopPropagation();
    setForm({ name: p.name, category: p.category||"Google Ads", client_name: p.client_name||"", color: p.color||"#c97d2a", budget: p.budget||"", received: p.received||"" });
  ...
  const saveProject = async () => {
    ...
    setSaving(true);
    const data = { name: form.name, category: form.category, client_name: form.client_name, color: form.color, budget: Number(form.budget)||0, received: Number(form.received)||0 };
  ...
  // Category counts for the active filter label
        ...
        <div className="toolbar" style={{ marginBottom: 18 }}>
          ...
          {categoryFilter !== "All" && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", background: CAT_COLORS[categoryFilter]?.bg, border: `1px solid ${CAT_COLORS[categoryFilter]?.color}`, borderRadius: 10 }}>
              ...
              <button onClick={() => setCategoryFilter("All")} style={{ background: "none", border: "none", cursor: "pointer", color: CAT_COLORS[categoryFilter]?.color, fontSize: 14, lineHeight: 1, padding: "0 2px" }}>×</button>
            </div>
          )}
              ...
              {sorted.map((p, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Billed */}
                    <td style={{ padding: "14px 16px", fontSize: 13, fontWeight: 600, color: "var(--blue)" }}>
                      {fmt(billed)}
                    </td>

                    {/* Received */}
                    <td style={{ padding: "14px 16px", fontSize: 13, fontWeight: 600, color: "var(--green)" }}>
                      {fmt(recvd)}
                    </td>
              ...
              <tfoot>
                <tr style={{ borderTop: "2px solid var(--border2)", background: "var(--surface2)" }}>
                  <td colSpan={3} style={{ padding: "12px 16px", fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                    {categoryFilter !== "All" ? `${categoryFilter} Total` : "Total"} — {sorted.length} projects
                  </td>
